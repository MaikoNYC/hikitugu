...
export default function SettingsPage() {
  const { user, session, authStatus, refreshAuthStatus } = useAuth();
  ...
  const handleSlackConnect = () => {
    ...
    window.location.href = `${API_URL}/api/auth/slack?token=${session.access_token}`;
  };

  const googleConnected = authStatus?.google?.connected ?? false;
  const slackConnected = authStatus?.slack?.connected ?? false;
